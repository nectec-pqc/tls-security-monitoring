...
def test_different_application_protocol_is_allowed(session):
    make_endpoint(session, application_protocol = 'ftp')
    make_endpoint(session, application_protocol = 'smtp')
...
def test_different_path_is_allowed(session):
    make_endpoint(session, path='/')
    make_endpoint(session, path='/api/v1')
...
def test_duplicate_endpoint_tuple_is_allowed(session):
    # The endpoint table has no uniqueness constraint, so identical endpoints
    # may coexist (deduplication is handled at discovery time, not by the DB).
